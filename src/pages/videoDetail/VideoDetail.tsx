    ...
    <div className={`${style.detailWrap}`}>
      <div className={style.mainContent}>
        <div className={style.contentHeader}>
          <div className={style.videoTitle}>{videoDetail?.name}</div>
          <div className={style.videoInfo}>
            <span className={style.infoItem}>
              发布：{formatTime(videoDetail?.publishTime || '', 'YYYY-MM-DD')}
            </span>{' '}
            <span className={style.infoItem}>
              播放：{formatNumber(videoDetail?.playCount || 0)}
            </span>{' '}
          </div>
        </div>
        <VideoPlayer
        ...
        <div className={`${style.videoBaseInfo} `}>
          <div className={style.artists}>
            {videoDetail?.artists?.map((item, index: number) => {
              return (
                <div
                  onClick={() => {
                    handleToAr(item.id)
          ...
          {videoDetail?.desc && (
            <div className={`${style.descWrap} ${showDesc ? style.descMore : ''}`}>
              {splitN(videoDetail?.desc)?.map((item) => {
                return (
